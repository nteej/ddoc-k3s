...
class ExtractJwtClaimsMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Custom JWT issued by user-service — fast path, no network call needed.
        // Kong already validated the signature at the gateway level.
        if (($payload['iss'] ?? null) === 'user-service') {
            $request->attributes->set('loggedUser', $payload);
            return $next($request);
        }

        // Passport Bearer token — validate via token introspection.
        // Only reachable when a client sends a Passport access token directly
        // (e.g., M2M integrations or third-party OAuth2 clients).
